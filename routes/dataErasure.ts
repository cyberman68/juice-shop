...
router.post('/', async (req: Request<Record<string, unknown>, Record<string, unknown>, DataErasureRequestParams>, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    if (req.body.layout) {
      const allowedDirectories = ['views', 'templates']
      const layoutPath = path.normalize(req.body.layout).replace(/^(\.\.(\/|\\|$))+/, '')
      const filePath = path.resolve(layoutPath).toLowerCase()
      const isForbiddenFile: boolean = (filePath.includes('ftp') || filePath.includes('ctf.key') || filePath.includes('encryptionkeys'))
      const isValidPath = allowedDirectories.some(dir => filePath.startsWith(path.resolve(dir)))
