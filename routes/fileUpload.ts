...
function handleZipFileUpload ({ file }: Request, res: Response, next: NextFunction) {
  if (utils.endsWith(file?.originalname.toLowerCase(), '.zip')) {
    if (((file?.buffer) != null) && utils.isChallengeEnabled(challenges.fileWriteChallenge)) {
      ...
      const tempFile = path.join(os.tmpdir(), cleanFilename)
          ...
          fs.close(fd, function () {
            fs.createReadStream(tempFile)
              ...
              .on('entry', function (entry: any) {
                ...
                const cleanFileName = path.basename(fileName);
                const absolutePath = path.resolve('uploads/complaints/' + cleanFileName)
                challengeUtils.solveIf(challenges.fileWriteChallenge, () => { return absolutePath === path.resolve('ftp/legal.md') })
                if (absolutePath.includes(path.resolve('uploads/complaints'))) {
                  entry.pipe(fs.createWriteStream('uploads/complaints/' + cleanFileName).on('error', function (err) { next(err) }))
                } else {
                  entry.autodrain()
                }
