...
export const status = function status () {
  return async (req: Request, res: Response, next: NextFunction) => {
    if (bot == null) {
      res.status(200).json({
        status: false,
        body: purify.sanitize(`${config.get<string>('application.chatBot.name')} isn't ready at the moment, please wait while I set things up`)
      })
      return
    }
    ...
    if (!token) {
      res.status(200).json({
        ...
        body: purify.sanitize(`Hi, I can't recognize you. Sign in to talk to ${config.get<string>('application.chatBot.name')}`)
      })
      return
    }
    ...
    const username = purify.sanitize(user.username);

    if (!username) {
      res.status(200).json({
        action: 'namequery',
        body: purify.sanitize('I\'m sorry I didn\'t get your name. What shall I call you?')
      })
      return
    }

    try {
      bot.addUser(purify.sanitize(`${user.id}`), username)
      res.status(200).json({
        status: bot.training.state,
        body: bot.training.state ? bot.greet(purify.sanitize(`${user.id}`)) : purify.sanitize(`${config.get<string>('application.chatBot.name')} isn't ready at the moment, please wait while I set things up`)
      })
    } catch (err) {
      next(new Error('Blocked illegal activity by ' + purify.sanitize(req.socket.remoteAddress)))
    }
  }
}
